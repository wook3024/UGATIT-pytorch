...
def extension_check(file_name):
    ...
    possible = any(file_name_lower.endswith(image_extention) for image_extention in image_extentions)
    return possible
...
image_dirs = make_dataset(root_dir)
...
for index, image_dir in enumerate(image_dirs):
    ...
    if not control_IN_LN_ratio: ratio_control_value = 0.0
    if not control_style: style_control_value = 1.0
    # ========================= original image ========================= #
    original_image = Image.open(target_dir if control_IN_LN_ratio or control_style else image_dir).convert('RGB')
    ...
    resp = requests.post(f"http://{ip_address}:{port}/predict", files={
        "image": open(target_dir if control_IN_LN_ratio or control_style else image_dir, 'rb'), 
...
#         "style_control": f'{style_control}'.encode('utf-8')})
#     translation_image_arr = np.array(resp.json()['image'], dtype=np.uint8)
#     # translation_image = Image.fromarray(translation_image_arr)
...
#     # separate to 2 rows
#     # if height > 512: concat_image_set = np.concatenate((concat_image_set[:, :height//2, :], concat_image_set[:, height//2:, :]), axis=0)
#     plt.imshow(translation_image_arr)
#     plt.show()
